...
/*   Updated: 2023/08/20 23:46:05 by bluque-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_map_element_node *destination_node, int image_index)
{
	...
	mlx_put_image_to_window(vars->mlx_ptr, vars->win_ptr,
		vars->images[image_index].image, destination_node->x * TILE_SIZE,
...
void	move_character(t_vars *vars, int dx, int dy, int image_index)
{
	...
	if (destination_node && destination_node->element == 'E'
		...
		handle_move(vars, player_node, destination_node, image_index);
